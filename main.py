...
def _display_placeholder_model(
    ...
    st.subheader(f"Summary generated by {model_visible_name}")          
    ...
    def _highlight(sent_idx, gen):
        ...
        with placeholder_selected.container():
            st.info(f"You selected the following sentence generated by {model_visible_name}. Highlight the parts in the sentence that can be found in the ground-truth abstract.")
            ...
            left, _, right = st.columns([3, 6, 3])

            def activate_checkbox(sent_idx_to_activate):
                st.session_state[f'chk_{model_name}_{doc_id}_{sent_idx}'] = False 
                st.session_state[f'chk_{model_name}_{doc_id}_{sent_idx_to_activate}'] = True 

            with left:
                if sent_idx > 0:
                    st.button('Previous sentence', on_click=activate_checkbox, args=(sent_idx-1,))
            with right:
                if sent_idx < len(gen_samples[doc_id]) - 1:
                    st.button('Next sentence', on_click=activate_checkbox, args=(sent_idx+1,))
        return highlighted
...
def loralay_eval_interface(
    ...
    with st.expander("Evaluation guidelines ", expanded=True):
        st.write("""
            Welcome! You are going to evaluate summaries generated by two models (A and B). For each document, you can find below its ground-truth abstract and the summaries - split by sentence - generated from the corresponding article.\\
                The instructions are as followed:\\
                    ...
                    \t2) In the ground-truth abstract, highlight the information that can be found in the sentence. This will define the recall score for the generated summary.\\
                    \t3) In the sentence, highlight the information that can be found in the ground-truth abstract. This will define the precision score for the generated sentence.\\
