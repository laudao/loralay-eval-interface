...
def _display_placeholder_model(
    ...
    def _update_gen_checkbox(sent_idx, model_name):
    ...
    def _update_prec_eval(sent_idx, model_name, results_dir):
        ...
        st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_prec_updated'] = st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_prec'] 
        with open(os.path.join(results_dir, f"{model_name}_{doc_id}_sent{sent_idx}_prec"), 'a') as fw:
            fw.write(str(st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_prec']) + "\n")

    def _update_rec_eval(sent_idx, model_name, results_dir):
        if f"{model_name}_{doc_id}_sent{sent_idx}_rec_updated" not in st.session_state:
            st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_rec_updated'] = st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_rec'] 
        st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_rec_updated'] = st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_rec'] 
        with open(os.path.join(results_dir, f"{model_name}_{doc_id}_sent{sent_idx}_rec"), 'a') as fw:
            fw.write(str(st.session_state[f'{model_name}_{doc_id}_sent{sent_idx}_rec']) + "\n")

    for sent_idx, sent in enumerate(gen_samples[doc_id]):
        first_col, second_col, third_col, fourth_col = st.columns([1, 5, 2, 2])
        with first_col:
            if sent_idx == 0:
                st.markdown("\n")
                st.markdown("\n")
        ...
        with third_col:
            st.selectbox(
                ...
                args=(sent_idx, model_name, prec_results_dir),
...
def load_results_in_session_state(slider_results_dir, prec_results_dir, rec_results_dir):
    ...
    for filename in rec_results_files:
        ...
        with open(os.path.join(rec_results_dir, filename), 'r') as f:
            lines = f.readlines()
        last_result = float(lines[-1])
        st.session_state[f'{model_name}_{doc_id}_{sent_idx}_rec_updated'] = last_result
    ...
    def prepare_results_dir():
        if os.path.isdir(args.results_dir):
            if args.overwrite_eval:
                if  (
                    len(os.listdir(slider_results_dir)) > 0 or len(os.listdir(prec_results_dir)) > 0
                ):
